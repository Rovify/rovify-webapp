...
export default function EventCard({ event, variant = 'default' }: EventCardProps) {
    ...
    return (
        <Link href={`/events/${event.id}`}>
            <div
                className={`group relative rounded-xl overflow-hidden ${isFeatured ? 'h-96' : isCompact ? 'h-32' : 'h-64'
            ...
            >
                ...
                <div className={`absolute bottom-0 left-0 right-0 z-20 p-4 ${isFeatured ? 'pb-6' : isCompact ? 'pb-3' : 'pb-4'
                    ...
                    <div className="space-y-1">
                        ...
                        {/* Title */}
                        <h3 className={`text-white font-bold ${isFeatured ? 'text-2xl' : isCompact ? 'text-base line-clamp-1' : 'text-lg'
                            } line-clamp-2`}>
                            {event.title}
                        </h3>
