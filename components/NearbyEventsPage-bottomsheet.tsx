...
// Enhanced iOS-Style Share Sheet
const ShareSheet = ({
    event,
    isOpen,
    onClose
    ...
    const shareOptions = [
        {
            id: 'native',
            icon: <IoShareOutline size={24} />,
            label: 'Share',
            color: 'bg-blue-500',
            action: () => event && shareEvent(event, 'native')
        },
        {
            id: 'copy',
            icon: <FiCopy size={24} />,
            label: 'Copy Link',
            color: 'bg-gray-500',
            action: () => event && shareEvent(event, 'copy')
        },
        {
            id: 'whatsapp',
            ...
            label: 'WhatsApp',
            color: 'bg-green-500',
            action: () => event && shareEvent(event, 'whatsapp')
        },
        {
            id: 'sms',
            ...
            label: 'Messages',
            color: 'bg-blue-600',
            action: () => event && shareEvent(event, 'sms')
        },
        {
            id: 'calendar',
            ...
            color: 'bg-red-500',
            action: () => event && addToCalendar(event)
        },
    ];
            ...
            {isOpen && (
                <>
                    ...
                    <motion.div
                        ...
                        className="fixed bottom-0 left-0 right-0 z-50 bg-white rounded-t-[28px] shadow-2xl max-w-lg mx-auto"
                    ...
                    >
                        ...
                        <div className="px-4 pb-6">
                            <div className="grid grid-cols-5 gap-3">
                                {shareOptions.map((option, index) => (
                                    ...
                                    >
                                        <div className={`w-14 h-14 ${option.color} rounded-2xl flex items-center justify-center text-white shadow-sm`}>
                                            {option.icon}
                                        </div>
                        ...
                        <div className="p-4">
                            <motion.button
                                ...
                                whileTap={{ scale: 0.99 }}
                                onClick={onClose}
                                className="w-full py-4 bg-white text-blue-500 font-semibold text-lg rounded-2xl border border-gray-200 hover:bg-gray-50 transition-colors"
                            >
                                Cancel
                            </motion.button>
                        </div>
...
// Real Interactive Map Implementation
const InteractiveMap = ({
    ...
    const getMarkerPosition = (event: NearbyEvent, index: number) => {
        const baseX = 20 + (index % 4) * 20;
        const baseY = 20 + Math.floor(index / 4) * 25;
        const zoomFactor = mapState.zoom / 13;
        return {
            left: `${baseX + mapOffset.x * 0.1 * zoomFactor}%`,
            top: `${baseY + mapOffset.y * 0.1 * zoomFactor}%`
        };
    };

    return (
        <div className="relative h-[70vh] w-full rounded-2xl overflow-hidden shadow-2xl border border-gray-200 bg-slate-100">
            {/* Map Container */}
            <div
                ref={mapRef}
                ...
                style={{
                    backgroundImage: `
                        ...
                        linear-gradient(-45deg, transparent 75%, #e2e8f0 75%)
                    `,
                    backgroundSize: `${20 * (mapState.zoom / 13)}px ${20 * (mapState.zoom / 13)}px`,
                    backgroundPosition: `0 0, 0 ${10 * (mapState.zoom / 13)}px, ${10 * (mapState.zoom / 13)}px -${10 * (mapState.zoom / 13)}px, -${10 * (mapState.zoom / 13)}px 0px`,
                    backgroundColor: '#f1f5f9',
                    transform: `scale(${mapState.zoom / 13})`,
            ...
            >
                ...
                {events.map((event, index) => {
                    const position = getMarkerPosition(event, index);
                    ...
                    return (
                        <motion.div
                            ...
                            className={`absolute cursor-pointer transform -translate-x-1/2 -translate-y-1/2 ${mapState.selectedEvent === event.id ? 'z-50' : 'z-10'
                                }`}
                            style={position}
                            onClick={(e) => {
                                e.stopPropagation();
                                updateMapState({ selectedEvent: event.id });
                            }}
                        >
                            {/* Marker Pin */}
                            <div className={`relative`}>
                                <div
                                    className={`w-8 h-8 rounded-full border-3 border-white shadow-lg flex items-center justify-center text-white text-xs font-bold ${category?.activeBg || 'bg-orange-500'
                                        }`}
                                >
                                    {event.price.replace('$', '')}
                                </div>
                                ...
                                {/* Pin Point */}
                                <div className={`absolute -bottom-1 left-1/2 transform -translate-x-1/2 w-0 h-0 border-l-4 border-r-4 border-t-4 border-transparent ${category?.activeBg?.replace('bg-', 'border-t-') || 'border-t-orange-500'
                                    }`} />
                                ...
                                {mapState.selectedEvent === event.id && (
                                    <>
                                        <motion.div
                                            ...
                                            className={`absolute inset-0 rounded-full ${category?.activeBg || 'bg-orange-500'}`}
                                        />
                                        <motion.div
                            ...
                            <motion.div
                                ...
                                className="absolute bottom-full left-1/2 transform -translate-x-1/2 mb-2 bg-black/80 text-white text-xs px-2 py-1 rounded whitespace-nowrap pointer-events-none"
                            >
                                {event.title}
                            </motion.div>
                        </motion.div>
            ...
            <AnimatePresence>
                {selectedEvent && (
                    <motion.div
                        ...
                        animate={{ opacity: 1, y: 0 }}
                        exit={{ opacity: 0, y: '100%' }}
                        className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl shadow-2xl border-t border-gray-200 p-6"
                    >
                        ...
                        <div className="flex items-center gap-4">
                            ...
                            <div className="flex-1 min-w-0">
                                <h3 className="font-bold text-gray-900 text-xl mb-1">{selectedEvent.title}</h3>
                                <p className="text-gray-600 text-sm mb-2">{selectedEvent.date} • {selectedEvent.location}</p>
                                <div className="flex items-center gap-3">
                                    <span className="text-orange-600 font-bold text-lg">{selectedEvent.price}</span>
                                    <span className="text-gray-500 text-sm">{selectedEvent.distance}</span>
                        ...
                        <div className="flex gap-3 mt-4">
                            <Link href={`/events/${selectedEvent.slug}`} className="flex-1">
                                <motion.button
                                    whileHover={{ scale: 1.02 }}
                                    whileTap={{ scale: 0.98 }}
                                    className="w-full py-3 bg-orange-500 text-white font-semibold rounded-2xl hover:bg-orange-600 transition-colors"
                                >
                                    View Details
                                </motion.button>
                            </Link>
            ...
            <Link href={eventUrl} className="flex flex-col sm:flex-row">
                ...
                <div className="p-6 flex-1">
                    <div className="flex items-start justify-between mb-2">
                        <h3 className="font-bold text-gray-900 text-lg mb-1 group-hover:text-orange-600 transition-colors flex-1">
                            {event.title}
                        </h3>
                        <span className="text-orange-600 font-bold text-lg">{event.price}</span>
                    </div>
                    ...
                    <div className="flex items-center justify-between">
                        ...
                        <motion.button
                            ...
                            className="w-10 h-10 rounded-full bg-orange-100 flex items-center justify-center text-orange-600 group-hover:bg-orange-500 group-hover:text-white transition-all duration-200"
                        >
